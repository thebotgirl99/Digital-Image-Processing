# -*- coding: utf-8 -*-
"""
Created on Fri Feb  5 16:19:01 2021
...
image = cv2.imread("C:/Users/HP/Desktop/calvinHobbes.jpeg", 1) #Read the image calvinHobbes from the path where it is
...
new_image= rotate_image(image, 30) #save the image returned by the rotate_image function in variable new_image and specify the image you read and angle you want to rotate by
near_img = cv2.resize(new_image,None, fx =2.5 , fy =2.5, interpolation = cv2.INTER_NEAREST) #Resize the image to (100,100) using nearest neighbor interpolation

bilinear_img = cv2.resize(new_image,None,fx =2.5, fy=2.5 , interpolation = cv2.INTER_LINEAR) #Resize the image to (100,100) using bilinear interpolation

bicubic_img = cv2.resize(new_image,None, fx =2.5, fy =2.5, interpolation = cv2.INTER_CUBIC) # #Resize the image to (100,100) using bicubic interpolation

height, width = new_image.shape[:2]
half_height, quater_width = height/2, width/4
T = np.float32([[1,0,quater_width],[0,1,half_height]])
img_translation=cv2.warpAffine(new_image,T,(width,height))

cv2.imshow('ImageWindow', img_translation) #display the image
